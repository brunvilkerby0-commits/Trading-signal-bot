...
const INTERVAL = "5min";
const MIN_SCORE = 70; // Score ranje nan 70 pou w ka jwenn siyal pi souvan
...
function analyze(candles) {
  ...
  if (ema9 > ema21) { callScore += 20; reasonsCall.push("EMA 9 > EMA 21"); }
  if (ema9 < ema21) { putScore += 20; reasonsPut.push("EMA 9 < EMA 21"); }
...
async function generateSignal(symbol) {
  try {
    ...
    const action = analysis.direction === "CALL" ? "CALL / BUY" : "PUT / SELL";

    const message = `🚨 <b>SMARTASSET POCKET OPTION SIGNAL</b>\n\n` +
      `💱 <b>PAIR:</b> ${symbol}\n\n` +
...
async function scheduler() {
  ...
  const isPreparationMinute = minute % 5 === 4;
  if (!isPreparationMinute) return;

  const slot = `${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}-${now.getUTCHours()}-${minute}`;
...
async function startup() {
  if (TWELVE_DATA_API_KEY && TELEGRAM_BOT_TOKEN) {
    await sendTelegram(
      ...
      "Bot la aktif pou voye siyal sou mache a."
    );
  }
}
