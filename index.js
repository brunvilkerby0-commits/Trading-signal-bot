...
// ==========================================
// ⏱️ MINITYE BOUJI M5 (35 segonn davans nan 4m25s)
// ==========================================
function demareMinitye() {
    ...
    setInterval(async () => {
        ...
        // Declenche nan 4m25s, 9m25s, 14m25s... (35 segonn anvan pwochen bouji an)
        if (minit % 5 === 4 && segonn === 25) {
...
async function prepareEpiVoye(symbol, aksyon) {
    // Geins 35 segonn nan tan an pou vize minit pil k ap vini an
    const antreDate = new Date(Date.now() + 35000);
    
    // Fòma 12 èdtan ak AM/PM nan zòn lokal Ayiti / New York
    const lèFormate = antreDate.toLocaleTimeString('en-US', {
    ...
    const mesaj = `🚨 *SIYAL TEST DEMO (M5)*\n\n` +
                  `📊 *Pè:* ${symbol}\n` +
                  `🏦 *Platfòm:* Pocket Option / Quotex\n` +
                  `🎯 *Aksyon:* ${ikon}\n` +
                  `⏱️ *Echeyans:* 5 Minit\n` +
                  `⏰ *Lè pou antre:* ${lèFormate}\n\n` +
                  `⏳ _Ou gen 35 segonn pou prepare pè sa a sou kont ou!_`;
