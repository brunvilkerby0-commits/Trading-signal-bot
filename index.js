...
// ==========================================
// 🌐 SÈVÈ HTTP AK PAJ WEB POU KÒD QR
// ==========================================
const app = express();
const PORT = process.env.PORT || 10000;

let currentQRImage = ''; // Moun ap ka wè kòd QR la la
        ...
        res.send(`
            <html>
                <head>
                    ...
                    <style>
                        body { display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh; font-family: sans-serif; background: #111; color: white; text-align: center; }
                        img { border: 10px solid white; border-radius: 10px; max-width: 80%; }
                    </style>
                </head>
                <body>
                    <h2>📲 Scan kòd QR sa a ak WhatsApp:</h2>
                    <img src="${currentQRImage}" alt="QR Code" />
                    <p>Relanse paj sa a si kòd la ekspire.</p>
                </body>
            </html>
        `);
    } else {
        res.send('<h2>🤖 Robot an konekte sou WhatsApp deja oswa kòd QR la poko pare!</h2>');
    }
});
...
const TARGET_GROUP_NAME = 'VIP trading';
...
async function demareBot() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (qr) {
            console.log('📲 Nouvo kòd QR jenere!');
            qrcode.generate(qr, { small: true });
            // Konvèti kòd QR an imaj pou paj web la
        ...
        if (connection === 'close') {
            ...
            currentQRImage = ''; // Efase QR la lè l fin konekte
            demareMinitye(sock);
        }
    });
}
