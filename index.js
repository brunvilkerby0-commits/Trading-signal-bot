...
app.get('/', (req, res) => {
    res.send('🤖 Bot Signal Smart Asset VIP k ap kouri nòmalman!');
});
...
// Lojik "Sèvo" pou simule analiz teknolojik sou mache a (RSI + Trend)
function analyzeMarketAndGetSignal() {
    // Lis pè ki disponib (Majò + OTC)
    const availablePairs = [
        'EUR/USD', 'GBP/USD', 'USD/JPY', 'AUD/USD', 'USD/CAD',
        'EUR/JPY', 'GBP/JPY', 'EUR/USD (OTC)', 'GBP/USD (OTC)', 'USD/JPY (OTC)'
    ];

    const selectedPair = availablePairs[Math.floor(Math.random() * availablePairs.length)];
    
    // Generasyon valè RSI ant 20 ak 80
    const simulatedRSI = Math.floor(Math.random() * 60) + 20; 
    let action = null;

    // Si RSI ba anpil (< 35): Mache an nan Oversold -> Bon pou BUY
    if (simulatedRSI <= 38) {
        action = 'BUY';
    } 
    // Si RSI wo anpil (> 65): Mache an nan Overbought -> Bon pou SELL
    else if (simulatedRSI >= 62) {
        action = 'SELL';
    } 
    // Si RSI an nan mitan, nou itilize tandans fò
    else {
...
function checkAndSendSignal() {
    ...
    // Executé Lojik Analiz Mache a
    const analysis = analyzeMarketAndGetSignal();

    // Kalkile Lè pou antre a (+35 segonn entèn) pou fòmatel san segonn
    const entryDate = new Date(localDate.getTime() + 35 * 1000);
    
    // Fòma 12h senp (egzanp: 10:45 AM)
    const timeOptions = { 
    ...
    const entryTimeFormatted = entryDate.toLocaleTimeString('en-US', timeOptions);

    // Voye Siyal la
    sendTelegramSignal(analysis.pair, analysis.action, entryTimeFormatted, analysis.rsi);
